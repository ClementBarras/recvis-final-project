...
if use_flow:
    print("using flow")
    train_set = ProxyTaskDataset(root=args.data, video_info_path=os.path.join(args.video_list_directory, 'completetrainlist_flow.txt'), use_flow=True)
    validation_set = ProxyTaskDataset(root=args.data, video_info_path=os.path.join(args.video_list_directory, 'completevallist_flow.txt'), use_flow=True)
else:
    train_set = ProxyTaskDataset(root=args.data, video_info_path=os.path.join(args.video_list_directory, 'completetrainlist.txt'))
    validation_set = ProxyTaskDataset(root=args.data, video_info_path=os.path.join(args.video_list_directory, 'completevallist.txt'))
...
def validation():
    ...
    for data, target in val_loader:
        ...
        # sum up batch loss
        criterion = torch.nn.CrossEntropyLoss(reduction='elementwise_mean')
        validation_loss += criterion(output.mean(2), target).data.item()
        # get the index of the max log-probability
        #score = output.data.mean(2)
        #mask = abs(score - 0.5) > 0.1
        #pred = score[mask].sum(2).max(1, keepdim=True)[1]
